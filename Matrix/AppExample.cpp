...
Status AppExample::save_rank(){
	...
	double sum_rank = 0.0;
	for(ID i = 1;i <= maxID;i++)sum_rank += idValueIn[i];
	cout<<"maxID: "<<maxID<<", sum_rank: "<<sum_rank<<", avg_rank: "<<sum_rank / maxID<<endl;
	return OK;
}
...
string id_to_str(ID rootID){
	std::stringstream buf;
	buf << rootID;
	return buf.str();
}

Status AppExample::visit_count(string path, unsigned rootNum, ID *root_arr, unsigned *res, unsigned *deg, unsigned *loop_vertices){
	int task_num = rootNum / UNIT_SIZE, remain = rootNum % UNIT_SIZE;
	size_t begin = 0;

	if(task_num){
		for(int i = 0;i < task_num;i++){
			CThreadPool::getInstance().AddTask(boost::bind(&AppExample::visit_count_task, this, path, begin, begin+UNIT_SIZE, root_arr, res, deg, loop_vertices));
			begin += UNIT_SIZE;
		}
	}
	if(remain)CThreadPool::getInstance().AddTask(boost::bind(&AppExample::visit_count_task, this, path, begin, rootNum, root_arr, res, deg, loop_vertices));
	CThreadPool::getInstance().Wait();
}

Status AppExample::visit_count_task(string path, unsigned begin, unsigned end, ID *root_arr, unsigned *res, unsigned *deg, unsigned *loop_vertices){
	deque<ID>* que = new deque<ID>();
	EntityIDBuffer* ent = new EntityIDBuffer();
	ent->setIDCount(1);
	ID curID;
	
	string fileName = string(path+"/visit."+id_to_str(root_arr[begin]));
	MMapBuffer *visit = new MMapBuffer(fileName.c_str(), maxID * sizeof(bool));
	bool *visit_arr = (bool *)(visit->get_address());
	
	for(;begin < end;begin++){
		if(!(!deg[root_arr[begin] * 2] && deg[root_arr[begin] * 2 + 1])){
			__sync_fetch_and_add(loop_vertices, 1);
			continue;
		}
		
		memset(visit_arr, false, maxID * sizeof(bool));
		visit_arr[root_arr[begin]] = true;
		res[root_arr[begin]]++;
		que->push_back(root_arr[begin]);
